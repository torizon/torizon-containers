...
setup_qt5() {
  ...
  if [[ "$PLATFORM_FILTER" == *am62p* ]]; then
    ...
    DOCKER_RUN=$(read-docker-run.sh "/runs/qt5-wayland-examples/qt5-wayland-examples-am67a-compose.run" "qt5-wayland-examples-am67a" "qt5-wayland-examples" "bash")
    ...
    DOCKER_RUN=$(read-docker-run.sh "/runs/qt5-wayland-examples/qt5-wayland-examples-imx93-compose.run" "qt5-wayland-examples-imx93" "qt5-wayland-examples" "bash")
  elif [[ "$PLATFORM_FILTER" == *imx95* ]]; then
    DOCKER_RUN=$(read-docker-run.sh "/runs/qt5-wayland-examples/qt5-wayland-examples-imx95-compose.run" "qt5-wayland-examples-imx95" "qt5-wayland-examples" "bash")
