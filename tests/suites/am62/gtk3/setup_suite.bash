setup_suite() {
    ...
    torizon/gtk3-tests-am62:stable-rc bash
}

teardown_suite() {
    docker container stop weston
    docker image rm -f $(docker container inspect -f '{{.Image}}' weston-am62)
    docker container rm weston

    docker container stop gtk3-tests
    docker image rm -f $(docker container inspect -f '{{.Image}}' gtk3-tests-am62)
