...
parse_docker_run() {
    ...
    if [[ -n "$container_name" ]]; then
        matched_command=$(echo "$matched_command" | sed "s/docker run/docker container run -d --name=$container_name --net=host/")
    else
        matched_command=$(echo "$matched_command" | sed "s/docker run/docker container run -d --net=host/")
    fi
